    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-accent/50 px-6 py-3 border-b border-border">
        <div className="flex items-center gap-2 text-sm">
          <button
            onClick={() => router.push("/client-portal/dashboard/finance/purchase-orders")}
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="mb-6 flex items-center justify-between">
          <Button variant="outline" onClick={() => router.push("/client-portal/dashboard/finance/purchase-orders")}>
              ...
              {purchaseOrderDetails.orderItems.map((item, idx) => (
                <div key={idx} className="p-4 bg-gray-50 rounded-lg border border-border">
                  <div className="grid grid-cols-4 gap-4">
                    <div>
                      <div className="text-sm text-muted-foreground mb-1">Quantity & UOM</div>
                    ...
                    <div>
                      <div className="text-sm text-muted-foreground mb-1">Direct</div>
                      <div className="font-semibold">{item.direct}</div>
                    </div>
                    <div className="flex items-end justify-end">
                      <div className="text-right">
                        <div className="text-sm text-muted-foreground mb-1">Unit Cost</div>
                        <div className="font-semibold">${item.unitCost.toFixed(2)}</div>
                      </div>
                      <div className="mx-3 text-2xl font-light text-muted-foreground">×</div>
                      <div className="text-right">
                        <div className="text-sm text-muted-foreground mb-1">Amount</div>
                        <div className="font-bold text-lg text-green-600">${item.amount.toFixed(2)}</div>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <Card className="p-6">
            <div className="max-w-md ml-auto space-y-3">
              <div className="flex justify-between text-lg">
                ...
                <span className="font-semibold">${purchaseOrderDetails.subtotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-lg">
                <span className="text-muted-foreground">GST (10%):</span>
                <span className="font-semibold">${purchaseOrderDetails.gst.toFixed(2)}</span>
              </div>
              <div className="pt-3 border-t-2 border-border flex justify-between">
                <span className="text-xl font-bold">Total:</span>
                <div className="text-right">
                  <div className="text-3xl font-bold text-green-600">${purchaseOrderDetails.total.toFixed(2)}</div>
